...
int main(int argc, char *argv[]) {
    ...
    //TENTATIVA 2 - DESTINO DO CLIENTE(ENVIANDO DADOS P/ PORTA 4000)
    // Zera a estrutura "from"
    bzero(&client, length);
    // Define a familia do endereco como do tipo Internet
    ...
    while (1) {
        ...
        n = recvfrom(sock, buf, BUFFER_SIZE, 0, (struct sockaddr *) &server, &fromlen);
        //printf ("Bytes recebidos: %d\n", n);
        if (n < 0) error("recvfrom");

        // Envia pacotes pela rede. Os parametros sao semelhantes aos da funcao recvfrom().
        n = sendto(sock, buf, BUFFER_SIZE, 0, (struct sockaddr *) &client, fromlen);
