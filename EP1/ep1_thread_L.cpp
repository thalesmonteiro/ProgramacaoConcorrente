...
mutex m; // Cria objeto de mutex p/ uso na operação de multiplicação das matrizes nas threads
...
void multiplica_matriz(int comeco, int lim){ //'comeco' define inicio da parte da thread, 'lim' indica o final da parte da thread
	m.lock(); // Inicia garantia que este bloco não acessará o mesmo espaço de memória que outras threads ao mesmo tempo
	...
	m.unlock(); // Libera operação que evita acesso de memória simultâneo entre threads
}
...
{	
	...
	for (int i = 0; i < NUM_THREADS; i++) { // Espera todas as threads encerrarem suas operações
